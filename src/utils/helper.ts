...
export const removeUnusedDate = (obj: any, additionalFields?: string[]) => {
  Object.keys(obj).forEach(
    (k) =>
      (obj[k] && typeof obj[k] === "object" && removeUnusedDate(obj[k])) ||
      ((["createdAt", "updatedAt"].includes(k) ||
        (additionalFields && additionalFields.includes(k))) &&
        delete obj[k])
  );
  return obj;
};
